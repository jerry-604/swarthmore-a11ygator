    ...
    <div className="p-5 dark:bg-black">
      ...
      {data.length === 0 ? (
        ...
        data.map((course, index) => (
          <div ref={el => sectionRefs.current[index] = el} data-index={(index + 1).toString()} key={index} tabIndex={0} aria-label={`${course.name} course-folder`} className="mt-12 mb-5 p-5 bg-white dark:bg-slate-800 border border-gray-500 dark:border-gray-600 shadow-xl rounded-lg dark:text-gray-200" id={course.name}>
            <h2 className="bg-orange-200 dark:bg-orange-600 text-center text-lg font-semibold py-3">{course.name}</h2>
                    ...
                    {item.children?.map((file, fileIndex) => (
                      file.name.endsWith('.html') && (
                        <Link href={`/${course.name}/${item.name}/${file.name}`} key={fileIndex} legacyBehavior>
                          <a id={file.name} className="block ml-5 mt-2 text-blue-800 dark:text-[#22d3ee] hover:text-blue-600 dark:hover:text-blue-300 hover:underline">{file.name}</a>
                        </Link>
                      )
                    ))}
                  </div>
